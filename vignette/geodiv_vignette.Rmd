...
Note that this step can be somewhat time intensive. For users that would like to run this example, but don't want to run this step, the code to import the rasters is included below.
...
# export as one
grid.arrange(plts[[1]], plts[[2]], plts[[3]], nrow = 2)
```
...
```
The code below performs the clustering analysis on the surface gradient metrics.
...
plot_clusters <- function(res.hc, nclust){
  # Plot colors
...
plot_clusters(res.hc_elev, nclust = 4)
plot_clusters(res.hc_evi, nclust = 3)
...
map_comps <- function(pc_dat, noNA_df, full_df, r, theme) {
  ...
  # Plot in a grid.
  grid.arrange(out_maps[[1]], out_maps[[2]], out_maps[[3]], out_maps[[4]], 
               out_maps[[5]], nrow = 2, ncol = 3) 
}
...
plot_loadings <- function(pc_dat) {
  ...
  grid.arrange(imp_plts[[1]], imp_plts[[1]], imp_plts[[1]], ncol = 3)
}
