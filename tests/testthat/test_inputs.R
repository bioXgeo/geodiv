...
test_that("incorrect inputs return errors in bearingarea", {
  expect_error(bearing_area('a'), 'x must be a raster.')
  expect_error(plot_ba_curve('a'), 'x must be a raster.')
  expect_error(plot_ba_curve(normforest, divisions = 'a'), 'divisions argument must be TRUE/FALSE.')
  expect_error(find_flat('a'), 'x must be a raster.')
  ...
  expect_error(height_ba('a'), 'x must be a raster.')
  ...
  expect_error(sdc('a', low = 0.1, high = 0.4), 'x must be a raster.')
  ...
  expect_error(sbi('a'), 'x must be a raster.')
  expect_error(svi('a'), 'x must be a raster.')
  expect_error(sci('a'), 'x must be a raster.')
  expect_error(sk('a'), 'x must be a raster.')
  expect_error(svk('a'), 'x must be a raster.')
  expect_error(spk('a'), 'x must be a raster.')
})
...
test_that("incorrect inputs return errors in fourier", {
  expect_error(std('a', plot = FALSE), 'x must be a raster.')
  expect_error(std(normforest, plot = 'a'), 'plot must be logical.')
  expect_error(srw('a', plot = FALSE), 'x must be a raster.')
  expect_error(srw(normforest, plot = 'a'), 'plot must be logical.')
  expect_error(sfd('a'), 'x must be a raster.')
})

test_that("incorrect inputs return errors in localsurface", {
  expect_error(findpeaks('a'), 'x must be a raster.')
  expect_error(findvalleys('a'), 'x must be a raster.')
  expect_error(ssc('a'), 'x must be a raster.')
  expect_error(sds('a'), 'x must be a raster.')
  expect_error(s10z('a'), 'x must be a raster.')
})

test_that("incorrect inputs return errors in sdq", {
  expect_error(sdq('a'), 'x must be a raster.')
  expect_error(sdq6('a'), 'x must be a raster.')
})
...
test_that("incorrect inputs return errors in surfacearea", {
  expect_error(flatsa('a'), 'x must be a raster.')
  expect_error(surface_area('a'), 'x must be a raster.')
  expect_error(sdr('a'), 'x must be a raster.')
})

test_that("incorrect inputs return errors in utilities", {
  expect_error(fitplane('a', order = 1), 'x must be a raster.')
  expect_error(fitplane(normforest, order = 'a'), 'order must be integer.')
  expect_error(fitplane(normforest, order = c(1, 2)), 'too many values supplied to order.')
  expect_warning(fitplane(normforest, order = 1), 'numeric order, converting to integer.')
  expect_error(fitplane(normforest, order = as.integer(0)), 'order must be >= 1.')
  expect_error(bestfitplane('a'), 'x must be a raster.')
  expect_error(remove_plane('a'), 'x must be a raster.')
})
